...
public class BooksRepository implements IBooks {
	...
	@Override
	public Iterable<Publisher> getPublisherByAuthor(String authorName) {
		Query query = em.createQuery("select p from Book b join b.publisher p join b.authors a where a.name=?1");
		query.setParameter (1, authorName);
		return query.getResultList();
	}
